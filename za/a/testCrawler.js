...
///////////////////////////////////////////////////////////
//here, we create a loop to go over each network
///////////////////////////////////////////////////////////
...
const retreiveData = async (fromBlock, toBlock) => {
  ...
  let logs = await alchemy.core.getLogs({
    fromBlock,
    toBlock,
    address: !evaluateIfWeHavePassedStatistics(fromBlock) ? oldEthernautAddress : newEthernautAddress, 
    topics: []
  });
  for (log of logs) {
    ...
    const topic0Array = web3.eth.abi.decodeParameter("address", String(log.topics[3])); 
    ...
    try {
      let data = {
        ...
        level: /*!evaluateIfWeHavePassedStatistics(toBlock) ? dataArray.level : */topic0Array.level
      };
      dataDrop.push(data);
      console.log(log);
    } catch (error) {
  ...
  console.log("jubilations! " + macroDataDrop.length + " logs filtered.");
...
console.log("old crawler did what it does best, and wrote us some filtered data! " + macroDataDrop.length + " to be precise!")
  ...
  const retreiveAndWrite = async () => {
    ...
    try {
      ...
      let nextToBlock = 7968901; //plus difference 3605, then 10,000 thereafter until 7968901
  ...
  /**then, architect the crawler for Sepolia, writing to  ../filteredSepoliaData*/
};
