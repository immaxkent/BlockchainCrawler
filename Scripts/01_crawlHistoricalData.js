...
const crawlHistoricalData = () => {
  ...
  const returnCurrentLevel = (fromBlock, txn) => {
    let result = "";
    if (!evaluateIfWeHavePassedReDeployment(fromBlock)) {
      let input = txn.data;
      let input_data = "0x" + input.slice(10);
      let oldLevelAddress = web3.eth.abi.decodeParameter("address", input_data);
      const levelsMapJson = fs.readFileSync(mappingDataPath);
      const levelsMap = JSON.parse(levelsMapJson);
      console.log(levelsMap);
      const getValueByKey = (object, address) => {
          return object.find(value => object[key] === address);
      }
      getValueByKey(levelsMap, oldLevelAddress);
      console.log(getValueByKey(levelsMap, oldLevelAddress));
    ...
    for (log of logs) {
      ...
      try {
        let data = {
          ...
          level: returnCurrentLevel(fromBlock, txn)
        };
        dataDrop.push(data);
      } catch (error) {
        console.log(error);
  ...
  console.log("jubilations! " + macroDataDrop.length + " " + network[i].name + " logs filtered.");
  }
