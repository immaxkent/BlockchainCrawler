...
const crawlHistoricalData = () => {
  for (let i = 0; i < 1; i++) {
    ...
    const web3 = new Web3();
    const mappingDataPath = `../Networks/${network[i].name}/levelsMapping.json`;
    const macroDataDrop = [];
    ...
    const retreiveData = async (fromBlock, toBlock) => {
      ...
      const returnCurrentLevel = (fromBlock, txn, log) => {
        let result = "";
        if (!evaluateIfWeHavePassedReDeployment(fromBlock)) {
          let input = txn.data;
          let input_data = "0x" + input.slice(10);
          let oldLevelAddress = web3.eth.abi.decodeParameter(
            "address",
            input_data
          );
          const levelsMapJson = fs.readFileSync(mappingDataPath);
          const levelsMap = JSON.parse(levelsMapJson);

          // console.log(
          //   "old level: ",
          //   oldLevelAddress,
          //   levelsMap[oldLevelAddress] + JSON.stringify(log)
          // );
          return levelsMap[oldLevelAddress];
        } else {
      ...
      let logs = await returnEndPointClient(i).getLogs({
        fromBlock,
        toBlock,
        address: !evaluateIfWeHavePassedReDeployment(fromBlock)
          ? oldEthernautAddress
          : newEthernautAddress,
        topics: [],
      });
      ...
      for (log of logs) {
        let txn = await returnEndPointClient(i).getTransaction(
          log.transactionHash
        );
        ...
        try {
          let data = {
            ...
            eventType:
              String(log.topics[0]) ===
              evaluateCurrentSolveInstanceHex(fromBlock)
                ? "LevelCompleted"
                : "InstanceCreated",
            blockNumber: log.blockNumber,
            timeStamp: block.timestamp,
            level: returnCurrentLevel(fromBlock, txn, log),
          };
          dataDrop.push(data);
        } catch (error) {
          console.log(error);
    ...
    const writeData = async () => {
      ...
      macroDataDrop.forEach((game) => {
        const { player } = game;
        const existingPlayerRecord = filteredData.find(
          (filteredGame) => filteredGame.player === player
        );
        const existingPlayerRecordIndex = filteredData.findIndex(
          (filteredGame) => filteredGame.player === player
        );
        ...
        if (!!existingPlayerRecord) {
          ...
          if (existingEntryForLevel && !existingEntryForLevel.isCompleted) {
            ...
            existingEntryForLevel &&
            existingEntryForLevel.isCompleted
          ) {
      ...
      fs.writeFileSync(filteredDataPath, JSON.stringify(filteredData), {
        flag: "a",
      });
      console.log(
        "old crawler did what it does best, and wrote us some filtered data! " +
          macroDataDrop.length +
          " to be precise!"
      );
    };
